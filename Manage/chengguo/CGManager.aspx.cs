...
{
    public partial class CGManager :WebBase
    {
        ...
        public void getData()
        {
            ...
            string sqlWhere = "and CGComponyid=" + CGComponyid + " ";
            int iCount = 0; //数据总量
            ...
            if (ViewState["sqlWhere"] == null)
                sqlWhere = "and CGComponyid=" + CGComponyid + " ";
            else
                sqlWhere = ViewState["sqlWhere"].ToString();
            DB bll = new DB();
            DataTable dt = bll.GetData(sqlWhere, pageindex * rowcount, rowcount, out iCount);
            if (dt != null)
            {
                grid.DataSource = dt;
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                ViewState["sqlWhere"] = "and CGComponyid=" + CGComponyid + " ";
                if (SeaUName.Value.Trim() != "")
                {
        ...
        protected void selectYear_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (selectYear.SelectedItem.Value != "")
                {
                    ViewState["sqlWhere"] = "CGComponyid=" + CGComponyid + " ";
                    sqlWhere += " and CGPubstate like '%" + selectYear.SelectedItem.Value + "%' ";
                    ViewState["sqlWhere"] = sqlWhere;
                    ViewState["pageIndex"] = 0;
                    getData();
                }
