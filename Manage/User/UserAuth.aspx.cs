...
namespace zscq.Manage.User
{
    public partial class UserAuth : System.Web.UI.Page
    {
        ...
        public void getData()
        {
            ...
            DataTable dt = UserDao.GetUserList(pageIndex, rowCount, -1, " where auth = 1 and UserType != 1");
            if (dt != null)
            {
                grid.DataSource = dt;
                grid.DataBind();
                int count = UserDao.GetUserListCount(pageIndex, rowCount, -1, " where auth = 1 and UserType != 1");
